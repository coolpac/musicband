...
### Загрузка видео для лайва
- 📖 [Быстрая инструкция по загрузке видео](docs/QUICK_VIDEO_UPLOAD.md)
- 📚 [Полное руководство по работе с видео](docs/VIDEO_UPLOAD_GUIDE.md)
- 🖥️ [Рекомендации по серверу и хостингу](docs/VIDEO_HOSTING_AND_SERVER.md)
...
# ✅ Склонирует проект
# ✅ Создаст .env с автоматически сгенерированными паролями
```
...
# Только frontend
./scripts/remote-deploy.sh --frontend

# Откат изменений
./scripts/remote-deploy.sh --rollback
```
...
- `SSH_KEY` - путь к приватному ключу (опционально)
